...
def make_file(video_name):
    ...
    video_state = {
        ...
        "ouput_path" : f"{base_path}\\{video_name}",
        "inpaint_ouput_path" : f"{base_path}\\inp_{video_name}",
        "face_images_path" : face_images_path,
        "bestface_img_path" : f"{base_path}\\best_faces",
        "seg_images_path" : seg_images_path,
        "seg_ouput_path" : f"{base_path}\\seg_{video_name}",
        "segweb_output_path" : f"{base_path}\\seg_web_{video_name}",
        "fps" : "",
        "total_frame" : "",
        "video_length " : "",
        "frame_size" : "",
...
def generate_video_from_frames(video_state):
    ...
    out = cv2.VideoWriter(video_state["seg_ouput_path"], fourcc, video_state["fps"], video_state["frame_size"])
    
    for painted_frame in painted_frame_files:
    ...
    convert_video_with_moviepy(video_state["seg_ouput_path"], video_state["segweb_output_path"])
